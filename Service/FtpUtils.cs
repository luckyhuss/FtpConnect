...
namespace FtpConnect.Service
{
    class FtpUtils
    {
        ...
        public static void Download(FtpClient ftpConnection)
        {
            ...
            ftpConnection.SetWorkingDirectory("/LIVRAISONS/_TRANSFERT");
            ...
            try
            {
                using (Stream s = ftpConnection.OpenRead(remoteFiles[0].FullName, FtpDataType.Binary))
                using (var remoteFile = File.Create(@"E:\t\ftp\" + remoteFiles[0].Name))
                {
                    // perform transfer
                    int length;
                    float bitRate = -1.0f;
                    ...
                    Console.WriteLine();
                    Console.WriteLine(String.Format("Downloading {0} of size {1} bytes ...", remoteFiles[0].Name, remoteFiles[0].Size.ToString("n", nfi)));
                    Console.WriteLine("===");
                    ...
                    do
                    {
                        ...
                        if (ftpConnection.IsConnected)
                        {
                            ...
                            remoteFile.Write(buffer, 0, length);

                            // send data in buffer to file system
                            remoteFile.Flush(true);
                            ...
                            double progressPercentage = totalByteTranferred / remoteFiles[0].Size * 100;
